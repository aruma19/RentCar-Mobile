...
class Book extends HiveObject {
  ...
  /// Validasi apakah booking bisa diselesaikan
  bool canBeCompleted() {
    return status == 'active' && paymentStatus == 'paid';
  }
  
  /// Validasi apakah booking bisa dibatalkan
  bool canBeCancelled() {
  ...
  bool canProcessPayment() {
    return !['cancelled'].contains(status) && paymentStatus != 'paid';
  }
  ...
  /// Update payment dengan validasi
    ...
    switch (newPaymentStatus) {
      case 'dp':
        if (amount < totalPrice * 0.5) {
          throw Exception('Jumlah DP minimal 50% dari total harga');
        }
  ...
  Book updateStatus(String newStatus) {
    ...
    switch (newStatus) {
      ...
      case 'completed':
        if (!canBeCompleted()) {
          throw Exception('Booking tidak dapat diselesaikan. Pastikan rental aktif dan sudah bayar lunas.');
        }
        break;
      case 'cancelled':
        if (!canBeCancelled()) {
        ...
        // Otomatis refund jika sudah bayar
        if (paidAmount > 0) {
          return copyWith(
            status: newStatus,
